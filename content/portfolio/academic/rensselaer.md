...
Primarily, the linearization process for spatial data uses Morton codes which are cheap to construct and provide decent spatial coherence. My extension to the LBVH construction was to take advantage of the Morton codes' ability to interleave arbitrarily sized integers. As long as there is an invertible mapping between the geometry data in its object space and the Morton code representing it within a global coordinate system, the LBVH construction can be performed in object space.
